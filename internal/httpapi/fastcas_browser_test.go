...
func TestFastCASBrowserAgainstProvider(t *testing.T) {
	...
	if issuer == "" || origin == "" {
		t.Skip("run from FastCAS browser contract")
	}
	parsed, err := url.Parse(origin)
	if err != nil || parsed.Host == "" {
		t.Fatal("invalid project origin")
	}
	...
	if response.StatusCode != 200 {
		t.Fatalf("FastTask frontend returned %d", response.StatusCode)
	}
