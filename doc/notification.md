...
- 生产容器（2026-09-24，`docker compose up -d`，`tech.md` §21.5）：真实生产库从 schema 8 迁移到 10 后，本表面在线上跑通——`/health/ready` 就绪，线上 OpenAPI 含 15 条通知路径，管理员登录后可读后台通道/接收端/投递记录（新表为空），普通用户可读自己的通道与接收端，前端产物与本地 `web/dist` 同一 bundle 哈希且包含「通知管理」页；`docker compose restart` 后计数与 schema 不变。**尚未在生产配置任何真实通道**，因此「生产环境已给真实用户发过通知」不在已验证范围内；第一条真实通道按上一条的做法先点「验证凭据」。
